      ...
      <div className={`h-full z-10 flex flex-col transition-all duration-300 border border-gray-200/50 backdrop-blur-xl bg-white/90 shadow-2xl rounded-3xl ${isCollapsed ? 'w-20' : 'w-80'}`}>
        ...
        <div className="p-4 border-b border-gray-200/50 flex flex-col space-y-4">
          ...
          <div className={`flex items-center ${isCollapsed ? 'justify-center' : 'justify-between'}`}>
            <Link to="/" className={`flex items-center group ${isCollapsed ? 'justify-center' : ''}`}>
              {isCollapsed ? (
                <img src="/logo-icon.svg" alt="TrackNGo" className="w-10 h-10 object-contain drop-shadow-md group-hover:scale-105 transition-transform" />
              ) : (
                <img src="/logo-full.svg" alt="TrackNGo" className="h-12 w-auto object-contain drop-shadow-md" />
              )}
            </Link>
